...
var handlerRedirect = function(route, serviceLocation){
  ...
  var handler = function(req, reply){
    var url = url.resolve(serviceLocation, req.url.path);
    var options = {
      headers: req.headers,
      payload: req.payload,
    };
    wreck[method](url, options, function(err, res, payload){
      if(err){
        return reply(err);
      }
  ...
  if(typeof(route.config.handler)==='function'){
    return route.config.handler = handler;
  }
  route.handler = handler;
};

Handlers.prototype.registerProxy = function(options){
  options.server = (options.server || []).map((route)=>handlerRedirect(route, options.serviceLocation));
  server.route(options.server);
  ui.register(options.ui, options.serviceLocation);
};

Handlers.prototype.register = function(options){
  var ui = this.ui;
  var server = this.server;
  if(options.uiOnly && options.serviceLocation){
    return this.prototype.registerProxy(options);
  }
  server.route(options.server);
